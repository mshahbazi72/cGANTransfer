...
def torch_calculate_frechet_distance(mu1, sigma1, mu2, sigma2, eps=1e-6):
  ...
  sigma1 += (torch.eye(*sigma1.size()) * 1e-6).float().cuda()
  sigma2 += (torch.eye(*sigma2.size()) * 1e-6).float().cuda()
...
# calc distance for KMMD calculation
...
# calculate kmmd with gaussian kernel
def calc_mmd(Mxx, Mxy, Myy, sigma):
    scale = Mxx.mean()
...
def prepare_inception_metrics(dataset, parallel, no_fid=False, kmmd=False):
  ...
  if kmmd:
    data_pool = torch.from_numpy(np.load(dataset+'_inception_features.npz')['pool']).cuda()
  ...
  def get_inception_metrics(sample, num_inception_images, num_splits=10, 
    ...
    KMMD = 9999
    if kmmd:
      Mxx = distance(data_pool, data_pool, False)
      Mxy = distance(data_pool, pool, False)
      Myy = distance(pool, pool, False)
      KMMD = calc_mmd(Mxx, Mxy, Myy, sigma=1)

    # Delete mu, sigma, pool, logits, and labels, just in case
    del mu, sigma, pool, logits, labels
    return IS_mean, IS_std, FID, KMMD
